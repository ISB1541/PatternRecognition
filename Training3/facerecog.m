...
for i=1:M
  ...
  S=[S temp];    % S 행렬에 모두 연결한다. 결과적으로 S[N1*N2,M] 행렬이 된다. 
end
pause;

% 빛과 배경에 의하여 발생하는 에러를 줄이기 위여여
% 설정된 평균과 분산을 기준으로 모든 이미지를 정규화한다. 
for i=1:size(S,2)       % for each image
  temp=double(S(:,i));
  m=mean(temp);
  st=std(temp);
...
%--------------------------------------------------------------------------
% 공분산행렬--> 고유값분석 --> 25개의 가장 큰 고유치에 해당하는 고유벡터 추출
%
% 공분산 행렬(Covariance matrix) C=A'A, L=AA'   
% (from L's eigenvectors to C's eigenvectors) 
%
A=dbx';
L=A*A';
% vv는 L의 고유벡터(eigenvector) dd는 L의 고유값(eigenvalue). 
[vv dd]=eig(L);
% 정렬하여 고유값이 제로인 것을 제거 
v=[];
d=[];
...
% 고유벡터의 정규화 :   v = v / sqrt(sum v_i*v_i)
...
% 학습 얼굴영상 집합에서 각 얼굴 dbx(:,i) 의 가중치를 계산 omega[25,i]
omega = [];
for h=1:size(dbx,2)
  WW=[]; 
  for i=1:size(u,2)
...
while (1)
InputImage0 = input('인식할 얼굴의 파일명과 확장자를 입력하세요 \n','s');
InputImage = imread(strcat('.\',InputImage0));
%InputImage = rgb2gray(InputImage0);
figure(5)
subplot(1,2,1)
